#!/usr/bin/env bash
# Run as a child shell: bash scripts/publish-beta.sh (do not source).
set -e
set -o pipefail
...
script2_work="$(mktemp -d "${TMPDIR:-/tmp}/script2-beta.XXXXXX")"
...
cd "$script2_work"
node -e 'const p=require("./package.json");if(p.name!=="homebridge-script2"||!/^1\.0\.0-beta\.\d+$/.test(p.version)||p.publishConfig?.tag!=="beta")throw Error("Expected Script2 1.0.0 beta metadata.");'
...
if ! cmp -s stable-before.txt stable-after.txt; then echo 'NOTICE: stable latest changed during this operation; inspect npm dist-tags. This script only published beta.'; fi
echo "Beta is ready. Select $script2_version in Homebridge UI to install it."
